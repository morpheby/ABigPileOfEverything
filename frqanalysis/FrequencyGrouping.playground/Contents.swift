...
//func randomGroupingV1<U,V>(of data: [U: Double], to target: [V: Double]) -> [V: [U]] {
//    var groupingQs =
...
//    var stepCoeff = 0.05
//    var accumulatedChange = Double.infinity
//    var totalError = Double.infinity
//    var lastTotalError = totalError
//
//    while accumulatedChange > cutoff && totalError > errorThreshold  {
//        accumulatedChange = 0.0
//        totalError = 0.0
//
//        let candidate = performGrouping(in: groupingQs, with: data)
//        for (targetKey, sumProb) in candidate {
//            let error = sumProb - target[targetKey]!
//            totalError += abs(error)
//            let sign = error.sign == .plus ? -1.0 : 1.0
//
//            groupingQs = Dictionary(uniqueKeysWithValues: groupingQs.map { dataKey, poss in
//                var possibilities = poss
//                // p ±= ∆f / (x * f'(p))
//                let p = possibilities[targetKey]!
//                let diff = sign * (error * stepCoeff / (Double(data.count) * data[dataKey]! * logistic_derivative(p)))
//                if abs(diff) < 20.0 {
//                    accumulatedChange += abs(diff)
//                }
//                possibilities[targetKey] = p + diff
////                if p + diff > 5.0 {
////                    possibilities[targetKey] = 5.0
////                } else if p + diff < -20.0 {
////                    possibilities[targetKey] = -20.0
////                }
...
//        groupingQs
//
//        let final = performGroupingFinal(in: groupingQs)
//        let finalError = final.map { targetKey, dataKeys in return abs(target[targetKey]! - (dataKeys.map { key in data[key]! } .reduce(0.0, +))) } .reduce(0.0, +)
//        finalError
//
//        lastTotalError = totalError
//    }
//
//    return performGroupingFinal(in: groupingQs)
//}
...
//            let error = probs.map { dataKey, p in p * logistic(data[dataKey]!) } .reduce(0.0, +) - target[targetKey]!
        ...
        if totalError < bestError {
            ...
            let final = performGroupingFinal(in: groupingQs)
            print(final)
            print(Dictionary(uniqueKeysWithValues: final.map { targetKey, dataKeys in (target[targetKey]!, dataKeys.map { key in data[key]! }) }))
            print(Dictionary(uniqueKeysWithValues: final.map { targetKey, dataKeys in (target[targetKey]!, dataKeys.map { key in data[key]! } .reduce(0.0,+)) }))
        }
